...
import com.example.myapplication.adapter.WhiteNoise_History_Adapter;
...
public class History_WhiteNoise_Fragment extends BaseLazyFragment implements WhiteNoise_History_Adapter.WhiteNoise_History_callBack {
    ...
    private WhiteNoise_History_Adapter listAdapter;
    ...
    private void initData() {
        ...
        listAdapter = new WhiteNoise_History_Adapter(getActivity());
        listAdapter.setWhiteNoise_History_callBack(this);
    ...
    private void ListData() {
        ...
        OkHttpUtil.postRequestNoDialog(Api.HEAD + "history/white_noise", map, new OkHttpUtil.OnRequestNetWorkListener() {
            @Override
    ...
    @Override
    public void whitenoise_click(Video_History_Bean.DataBean bean) {
        if(!TextUtils.isEmpty(bean.resource_url)){
            //直接播放
    ...
    private void getwhite_noise_detail(Video_History_Bean.DataBean bean) {
        HashMap<String, String> map = new HashMap<>();
        map.put("detail_id", String.valueOf(bean.resource_id));
            ...
            public void ok(String response, JSONObject jsonObject) {
                try {
                    ...
                    if (code == 200) {
                        ...
                        if (dataBean != null) {
                            //设置url
                            if (!TextUtils.isEmpty(dataBean.resource_url)) {
                                bean.resource_url = dataBean.resource_url;
                                //播放
                            }
                        }
                    } else {
